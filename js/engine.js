...
var Engine = (function(global) {
    ...
    function checkCollisions() {
        allEnemies.forEach(enemy => {
            let hadCollision =
                Math.abs(player.x - enemy.x) < 101 - 41 &&
                Math.abs(player.y - enemy.y) < 83 - 23;
            if (hadCollision) {
    ...
    function checkItemsCollected() {
        items.forEach(item => {
            const itemCollected =
                Math.abs(player.x - item.x) < 101 - 51 &&
                Math.abs(player.y - item.y) < 83 - 33;
            if (itemCollected) {
                if (item.name === "Heart") {
                    item.addLife();
                } else {
    ...
    function checkItemsCollision() {
        ...
        items.forEach(item => {
            ...
            for (let index = start; index < end; index++) {
                if (hadCollision(item, items[index + 1])) {
    ...
    function hadCollision(spriteA, spriteB) {
        const hadCollision =
            Math.abs(spriteA.x - spriteB.x) < 101 - 51 &&
            Math.abs(spriteA.y - spriteB.y) < 83 - 33;
        return hadCollision;
    }
